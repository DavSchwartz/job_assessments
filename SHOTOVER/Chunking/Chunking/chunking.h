...
#ifdef	CHUNKING_EXPORTS
#define CHUNKING_API __declspec(dllexport)
#else
#define CHUNKING_API __declspec(dllimport)
#endif
...
extern "C" CHUNKING_API SearchContext * create(const Point * points_begin, const Point * points_end);
...
extern "C" CHUNKING_API int32_t search(SearchContext * sc, const Rect rect, const int32_t count, Point * out_points);

/* Release the resources associated with the context. Return nullptr if successful, "sc" otherwise. */
extern "C" CHUNKING_API SearchContext * destroy(SearchContext * sc);
